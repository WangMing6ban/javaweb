...
<body>
<center><br />
<form action="xiugai.action" name="addform" id="addform"
	method="post"><br />
<br />
<table border="0">
	<tr>
		<td>图书名称</td>
		<td><input type="hidden" name="id" id="id" value="${book.id}" />
		...
		<td><select id="classid" name="classid">
			<c:forEach items="${bookType}" var="type">
				<option value="${type.id}">
					<c:if test="${type.id==book.type.id}"> selected="selected" </c:if>>
						${type.type_name}
				</option>
			</c:forEach>
			</select>
	...
	<tr>
		<td>图片</td>
		<td><input type="hidden" name="book_imgurl" value="${book.book_imgurl }"/>		
		</td>
	</tr>
	<tr>
		<td></td>
